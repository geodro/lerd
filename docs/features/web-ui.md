...
- **PHP-FPM cards** — show which sites use the version, Xdebug toggle with an inline mode selector (debug, coverage, debug,coverage, develop, profile, trace, gcstats — visible when Xdebug is on), custom extension list, and a live FPM log stream. For versions with no active sites, a manual Start/Stop button is shown.
